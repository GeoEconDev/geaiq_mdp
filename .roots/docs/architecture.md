...
metadata/{país}/*.yml
  ...
  → get_processor(source)               # BigQuery | Shape  (factory)
...
- `processors.py` — factory `get_processor(source)` por `(source.type, source.platform)`: `("sql", bigquery)`→BigQuery, `("sql", postgresql)`→**PostgreSQL (placeholder, NotImplementedError)**, `("shape", googledrive)`→Shape.
- `bigquery.py` — `BigQuerySourceProcessor.test_source()` dry-run + valida schema + estima costo; `run_query()` cacheable.
- `shape.py` — `ShapeProcessor`: baja ZIP de Drive (`MediaIoBaseDownload`), detecta encoding (chardet), retorna `GeoDataFrame`.
